...
# from keras.models.
'''
   这是一个《python深度学习》多分类模型，对车和导向线的有无进行分类，
   数据集用的道路行驶的图片
'''
...
best_model = ModelCheckpoint("car_line_3.h5", monitor='val_loss', verbose=0, save_best_only=True)# 随时保存训练好的模型参数
...
model.compile(loss='categorical_crossentropy',               # 使用的损失函数 二元交叉熵
              ...
              metrics=['acc'])                          # 监控精度
model.summary()
best_model = ModelCheckpoint("car_line_4.h5", monitor='val_loss', verbose=0, save_best_only=True) # 随时保存训练好的模型参数
history = model.fit_generator(              # 开始训练
    train_generator,                        # 训练模型使用的训练集图像生成器
    steps_per_epoch=281,                    # 从生成器中抽取 steps_per_epoch 个批量后，进入下次迭代
    epochs=70,                              # 迭代次数
